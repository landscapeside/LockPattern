...
public class LockPatternView extends View {
  ...
  private int defColor, selectColor, errColor;
    ...
    for (int i = 0; i < mCells.length; i++) {
      for (int j = 0; j < mCells[i].length; j++) {
        if (mCells[i][j].getStatus() == Cell.STATE_CHECK) {
          ...
          canvas.drawCircle(mCells[i][j].getX(), mCells[i][j].getY(), this.cellRadius,
              this.selectPaint);
          selectPaint.setStyle(Style.FILL);
          canvas.drawCircle(mCells[i][j].getX(), mCells[i][j].getY(), this.cellInnerRadius,
              this.selectPaint);
   ...
   */
  @Deprecated private void initViewSize(Context context, AttributeSet attrs) {
  ...
  /**
   * obtain colors:
   * 1.textColor represent normal color
   * 2.textHintColor represent error color
   */
  private void initColors(AttributeSet attrs) {
    TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.LockPattern);
    defColor = a.getColor(R.styleable.LockPattern_selectColor,
        getResources().getColor(R.color.blue_78d2f6));
    selectColor = defColor;
    errColor = a.getColor(R.styleable.LockPattern_errColor,
        getResources().getColor(R.color.red_f3323b));
  }
  ...
  private void init9Cells() {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        mCells[i][j] =
            new Cell(distance * j + cellRadius + offset, distance * i + cellRadius + offset, i, j,
                3 * i + j + 1);
      }
    }
  }
  ...
  private void initPaints() {
    ...
    defaultPaint.setColor(defColor);
   ...
   */
  @Deprecated private void drawLineIncludeCircle(Cell preCell, Cell nextCell, Canvas canvas,
      Paint paint) {
  ...
  private void drawLineNotIncludeCircle(Cell preCell, Cell nextCell, Canvas canvas, Paint paint) {
    float distance =
        LockPatternUtil.getDistanceBetweenTwoPoints(preCell.getX(), preCell.getY(), nextCell.getX(),
            nextCell.getY());
    ...
    float x2 = (distance - this.cellRadius) / distance * (nextCell.getX() - preCell.getX())
        + preCell.getX();
    float y2 = (distance - this.cellRadius) / distance * (nextCell.getY() - preCell.getY())
        + preCell.getY();
  ...
  private void drawLineFollowFinger(Cell preCell, Canvas canvas, Paint paint) {
    float distance =
        LockPatternUtil.getDistanceBetweenTwoPoints(preCell.getX(), preCell.getY(), movingX,
            movingY);
   ...
   */
  @Deprecated private void drawTriangle(Cell preCell, Cell nextCell, Canvas canvas, Paint paint) {
    float distance =
        LockPatternUtil.getDistanceBetweenTwoPoints(preCell.getX(), preCell.getY(), nextCell.getX(),
            nextCell.getY());
    float x =
        this.cellInnerRadius * 2 / distance * (nextCell.getX() - preCell.getX()) + preCell.getX();
    float y =
        this.cellInnerRadius * 2 / distance * (nextCell.getY() - preCell.getY()) + preCell.getY();

    float angleX =
        LockPatternUtil.getAngleLineIntersectX(preCell.getX(), preCell.getY(), nextCell.getX(),
            nextCell.getY(), distance);
    float angleY =
        LockPatternUtil.getAngleLineIntersectY(preCell.getX(), preCell.getY(), nextCell.getX(),
            nextCell.getY(), distance);
    float x1, y1, x2, y2;
    //slide right down
  ...
  private void addSelectedCell(Cell cell) {
    if (!sCells.contains(cell)) {
      if (sCells.size() > 0) {
        Cell lastCell = sCells.get(sCells.size() - 1);
        Cell centerCell = getCellBetweenTwoCells(lastCell, cell);
        if (centerCell != null && !sCells.contains(centerCell)) {
          centerCell.setStatus(Cell.STATE_CHECK);
          sCells.add(centerCell);
        }
      }
   ...
   */
  @Deprecated public List<Cell> setDefaultSelectedCell(String value) {
