...
public class LockPatternIndicator extends View {
  ...
  private int selectColor, normalColor;
  ...
  /**
   * obtain colors:
   * 1.textColor represent select color
   * 2.textHintColor represent normal color
   */
  private void initColors(AttributeSet attrs) {
    TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.LockPattern);
    selectColor = a.getColor(R.styleable.LockPattern_selectColor,
        getResources().getColor(R.color.blue_01aaee));
    normalColor = a.getColor(R.styleable.LockPattern_defColor,
        getResources().getColor(R.color.grey_b2b2b2));
  }
  ...
  private void initPaint() {
    ...
    defaultPaint.setColor(normalColor);
  ...
  private void init9IndicatorCells() {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        mIndicatorCells[i][j] =
            new IndicatorCell(distance * j + radius + offset, distance * i + radius + offset,
                3 * i + j + 1);
      }
    }
  }
    ...
    for (int i = 0; i < mIndicatorCells.length; i++) {
      for (int j = 0; j < mIndicatorCells[i].length; j++) {
        if (mIndicatorCells[i][j].getStatus() == IndicatorCell.STATE_NORMAL) {
          canvas.drawCircle(mIndicatorCells[i][j].getX(), mIndicatorCells[i][j].getY(), radius,
              defaultPaint);
